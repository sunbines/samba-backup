...
global_build_dir = $(build_dir)/global-6.5.6
config_src_dir = $(TOPDIR)/config
global_dir_name = global-6.5.6

######## talloc-2.4.2 ########################
talloc_src_dir = $(core_src_dir)/talloc-2.4.2
talloc_build_dir = $(build_dir)/talloc-2.4.2
talloc_dir_name = talloc-2.4.2
...
######## resource-agent ########################
resource_agents_src_dir = $(core_src_dir)/resource-agents-4.9.0
resource_agents_build_dir = $(build_dir)/resource-agents-4.9.0
resource_agents_dir_name = resource-agents-4.9.0


######## tdb-1.4.10 ########################
all:  .build_global .build_talloc .build_tdb .build_tevent .build_lmdb .build_ldb .build_resource_agents
...
.build_resource_agents:
	...
	@if [ -d $(build_dir) ]; then rm -rf $(resource_agents_build_dir); fi
	@mkdir -p $(resource_agents_build_dir)
	@(mkdir -p $(resource_agents_build_dir)/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS})
	@(cd $(resource_agents_build_dir)/SOURCES; \
			cp -a $(resource_agents_src_dir)/config/* $(resource_agents_build_dir)/SOURCES; \
			cp -a $(resource_agents_src_dir) $(resource_agents_dir_name); \
			tar -zcvf $(resource_agents_dir_name).tar.gz $(resource_agents_dir_name); rm -rf $(resource_agents_dir_name); )

	@echo "---------- copy spec ----------"
	@(cp -af $(resource_agents_src_dir)/resource-agents.spec $(resource_agents_build_dir)/SPECS)
	@echo "---------- build resource_agents ----------"
	@(rpmbuild -ba --define="_topdir $(resource_agents_build_dir)" $(resource_agents_build_dir)/SPECS/resource-agents.spec)
